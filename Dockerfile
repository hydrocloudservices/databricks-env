...
RUN ESMFMKFILE="$(find $ESMF_INSTALL_PREFIX -name '*esmf.mk')" \
    ...
    && /databricks/python3/bin/python setup.py build --ESMFMKFILE=${ESMFMKFILE} \
    && /databricks/python3/bin/python setup.py test \
    && /databricks/python3/bin/python setup.py install \
    && /databricks/python3/bin/python -c "import ESMF; print(ESMF.__file__, ESMF.__version__)" \
...
ENV PYSPARK_PYTHON=/databricks/python3/bin/python
